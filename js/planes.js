...
window.PlaneSystem = {
    canvas: null,
    ctx: null,
    container: null, // Для DOM элементов самолетов (svg)
    planes: [],
    nextSpawnTime: 0,
    ...
    init() {
        ...
        if (!planeDomLayer) {
            ...
            // ВАЖНО: pointer-events auto не ставим всему слою, иначе перекроет клики
            // Но нам нужно, чтобы клики проходили сквозь пустоту к коробке,
            // а попадали в самолет только если наведены на него.
